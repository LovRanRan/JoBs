    ...
    <div className="max-w-3xl">
      ...
      <div className="mt-6 grid grid-cols-2 gap-6">
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          ...
          <Row label="邮箱" value={p.email} />
          <Row label="当前职位" value={p.title} />
          <Row label="经验" value={`${p.yearsExp} 年`} />
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          ...
          <Row label="最低薪资" value={`$${p.minSalary.toLocaleString()}`} />
        </div>
      </div>
      ...
      <div className="mt-6 rounded-xl border border-gray-200 bg-white p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {p.skills.map((s) => (
            <span key={s} className="rounded-lg bg-brand-50 px-3 py-1 text-sm text-brand-700">
              {s}
            </span>
          ))}
        </div>
